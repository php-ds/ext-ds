...
extern zend_class_entry *php_ds_seq_ce;

ARGINFO_OPTIONAL_ZVAL(Seq___construct, values);
ARGINFO_NONE_RETURN_OBJ(Seq_getIterator, Traversable);

/**
 * Sequence method arginfo (formerly in php_sequence_ce.h).
 * These use Sequence_ prefix because the Collection arginfo macros
 * reference Collection_ prefix — both are shared via arginfo name.
 */
